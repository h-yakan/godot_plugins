...
func _dialogue_paused() -> bool:
	return pause_when_dialogue_overlay and GaDialogueManager and GaDialogueManager.dialogue_paused_by_overlay
...
func update_eye_visuals() -> void:
	...
	if not is_eyes_closed and current_lid_value >= 0.98:
		...
		if use_event_bus and GaEventBus:
			GaEventBus.eyes_toggle.emit()
			GaEventBus.eyes_closed.emit()
