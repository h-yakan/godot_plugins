...
static func preload_all() -> void:
	...
	preload(BASE + "content/sample/handlers/sample_resource_bonus_effect.gd")
	preload(BASE + "content/sample/handlers/sample_stat_multiply_effect.gd")
	preload(BASE + "content/sample/handlers/sample_option_pick_effect.gd")
	preload(BASE + "content/sample/sample_catalog.gd")
	preload(BASE + "content/sample/register_sample_pack.gd")
...
static func register_sample_pack(api: Object) -> void:
	preload_all()
	var pack_script: GDScript = load(BASE + "content/sample/register_sample_pack.gd")
	pack_script.register(api)


static func sample_content_dir() -> String:
	return BASE + "content/sample/perks"
