...
func _ready() -> void:
	...
	if dialogue_label:
		...
		dialogue_label.text = ""
	if GaEventBus:
		GaEventBus.ear_state_changed.connect(_on_ear_state_changed)
...
func _start_typewriter_effect(text: String, delay_per_character: float, speaker_id: String, color: Color = Color.WHITE) -> void:
	...
	for i in range(text.length()):
		...
		if not speaker_id.is_empty() and GaEventBus:
			GaEventBus.dialogue_character_typed.emit(speaker_id)
