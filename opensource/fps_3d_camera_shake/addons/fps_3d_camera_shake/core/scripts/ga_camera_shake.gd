...
const _EventBus := preload("res://addons/fps_3d_camera_shake/core/ga_event_bus_client.gd")
...
func _ready() -> void:
	...
	_EventBus.listen(self, &"camera_shake_trauma", _on_trauma_signal)
...
## Alternative: Event Bus `camera_shake_trauma` (requires Any Game Event Bus)
