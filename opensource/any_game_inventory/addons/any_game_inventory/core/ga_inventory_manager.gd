...
const _EventBus := preload("res://addons/any_game_inventory/core/ga_event_bus_client.gd")
...
func equip_item(slot_index: int) -> bool:
	...
	_EventBus.emit(self, &"equipped_changed", [equipped_item])
	return true

func unequip_item() -> void:
	equipped_item = null
	_EventBus.emit(self, &"equipped_changed", [null])
