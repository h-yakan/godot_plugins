...
func _ready() -> void:
	if GaEventBus:
		...
		GaEventBus.interactable_activated.connect(_on_interactable_activated)
	...
	for subquest_id in quest.subquests:
		if subquest_id in all_quests:
			...
			if _check_prerequisites(subquest):
				...
				if GaEventBus:
					GaEventBus.quest_started.emit(subquest_id)
	for event_id in quest.start_events:
		_trigger_event(event_id)
	if GaEventBus:
		GaEventBus.quest_started.emit(quest_id)
...
func update_quest_progress(quest_id: String, progress: int) -> void:
	...
	if quest.current_progress != old_progress and GaEventBus:
		GaEventBus.quest_progress_updated.emit(quest_id, quest.current_progress, quest.max_progress)
		...
		if next_quest:
			...
			for q in active_quests.values():
				if q.parent_quest_id.is_empty():
					...
					break
			if followed_quest_id.is_empty() and GaEventBus:
				GaEventBus.followed_quest_changed.emit("")
	for event_id in quest.completion_events:
		_trigger_event(event_id)
	if GaEventBus:
		GaEventBus.quest_completed.emit(quest_id)
...
func fail_quest(quest_id: String) -> void:
	...
	if GaEventBus:
		GaEventBus.quest_failed.emit(quest_id)
	if quest_ui and quest_ui.has_method("update_quests_display"):
		quest_ui.update_quests_display()
...
func set_followed_quest(quest_id: String) -> bool:
	...
	if GaEventBus:
		GaEventBus.followed_quest_changed.emit(quest_id)
	return true
