...
func _ready() -> void:
	if not quest_item_scene:
		quest_item_scene = preload("res://addons/any_game_quest_system/pro_features/scenes/ga_quest_item_ui.tscn")
	_EventBus.listen(self, &"quest_started", _on_quest_started)
	_EventBus.listen(self, &"quest_completed", _on_quest_completed)
	_EventBus.listen(self, &"quest_progress_updated", _on_quest_progress_updated)
