...
const _EventBus := preload("res://addons/any_game_progress_tracker_ui/core/ga_event_bus_client.gd")
...
func _ready() -> void:
	if not item_ui_scene:
		item_ui_scene = preload("res://addons/any_game_inventory/pro_features/ga_item_ui.tscn")
	if tooltip_scene:
		_tooltip = tooltip_scene.instantiate()
		add_child(_tooltip)
	_EventBus.listen(self, &"followed_quest_changed", _on_followed_quest_changed)
	_EventBus.listen(self, &"quest_started", _on_quest_started_or_completed)
	_EventBus.listen(self, &"quest_completed", _on_quest_started_or_completed)
	_EventBus.listen(self, &"dialogue_finished", _on_dialogue_finished)
	_EventBus.listen(self, &"inventory_changed", _on_inventory_changed)
	_EventBus.listen(self, &"equipped_changed", _on_equipped_changed)
