...
func _initialize_persistence() -> void:
	var script_path := "res://addons/clyde_dialogue_host/pro_features/services/dialogue_persistence.gd"
	if not enable_persistence or not ResourceLoader.exists(script_path):
		persistence = null
		enable_persistence = false
		return
	var persistence_script: GDScript = load(script_path)
	persistence = persistence_script.new()
...
func _initialize_dice() -> void:
	var script_path := "res://addons/clyde_dialogue_host/pro_features/services/dialogue_dice_roller.gd"
	if not enable_dice or not ResourceLoader.exists(script_path):
		dice_roller = null
		enable_dice = false
		return
	var dice_script: GDScript = load(script_path)
	dice_roller = dice_script.new()
