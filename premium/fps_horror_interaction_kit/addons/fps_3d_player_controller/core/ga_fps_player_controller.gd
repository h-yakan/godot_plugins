...
const _EventBus := preload("res://addons/fps_3d_player_controller/core/ga_event_bus_client.gd")
...
var health: float:
	set(value):
		...
		if is_equal_approx(clamped, _health):
			return
		_health = clamped
		_EventBus.emit(self, &"player_health_changed", [_health, max_health], use_event_bus)
		if _health <= 0.0:
			player_died.emit()
	get:
		return _health
...
func _ready() -> void:
	...
	_EventBus.extend(self, &"player_health_changed", use_event_bus)
	_EventBus.extend(self, &"player_look_switch", use_event_bus)
	_EventBus.extend(self, &"player_move_switch", use_event_bus)
	_EventBus.listen(self, &"player_look_switch", _on_player_look_switch, use_event_bus)
	_EventBus.listen(self, &"player_move_switch", _on_player_move_switch, use_event_bus)
