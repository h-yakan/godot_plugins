...
const _EventBus := preload("res://addons/3d_interactable_tv_viewport/core/ga_event_bus_client.gd")
...
func play_tv() -> void:
	...
	_EventBus.emit(self, &"player_move_switch", [false], use_event_bus)
	_EventBus.emit(self, &"player_look_switch", [false], use_event_bus)
	_EventBus.emit(self, &"toggle_ui", [], use_event_bus)
...
func _restore_from_tv() -> void:
	...
	_EventBus.emit(self, &"player_move_switch", [true], use_event_bus)
